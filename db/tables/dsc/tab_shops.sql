--------------------------------------------------------
--  DDL for Table SHOPS
--------------------------------------------------------

CREATE TABLE "IGTP"."SHOPS" (	
	"ID" 				NUMBER(8,0) DEFAULT IGTP.SHOPS_SEQ.NEXTVAL, 
	"SHOP_CO" 			VARCHAR2(6 BYTE), 
	"DESCRIPTION" 		VARCHAR2(80 BYTE), 
	"LOCATION_ID" 		NUMBER(8,0), 
	"ADDRESS" 			VARCHAR2(250 BYTE), 
	"NU_GPS_LAT" 		NUMBER(8,4), 
	"NU_GPS_LON" 		NUMBER(8,4), 
	"TELEPHONE_CO" 		VARCHAR2(50 BYTE), 
	"FAX_CO" 			VARCHAR2(50 BYTE), 
	"EMAIL" 			VARCHAR2(60 BYTE), 
	"NAME_CONTACT" 		VARCHAR2(20 BYTE), 
	"EMAIL_CONTACT" 	VARCHAR2(60 BYTE), 
	"TELEPHONE_CONTACT" VARCHAR2(20 BYTE), 
	"USER_ID" 			NUMBER(8,0), 
	"UUID" 				VARCHAR2(60 BYTE), 
	"SLUG" 				VARCHAR2(60 BYTE), 
	"CREATED_AT" 		DATE DEFAULT sysdate, 
	"UPDATED_AT" 		DATE DEFAULT sysdate
) 
SEGMENT CREATION IMMEDIATE 
	PCTFREE 10 
	PCTUSED 0 
	INITRANS 1 
	MAXTRANS 255 
	NOCOMPRESS LOGGING
	STORAGE(
		INITIAL 65536 
		NEXT 1048576 
		MINEXTENTS 1 
		MAXEXTENTS 2147483645
		PCTINCREASE 0 
		FREELISTS 1 
		FREELIST GROUPS 1
		BUFFER_POOL DEFAULT 
		FLASH_CACHE DEFAULT 
		CELL_FLASH_CACHE DEFAULT
	)
	TABLESPACE "TS_IGTP_DAT" ;

COMMENT ON TABLE "IGTP"."SHOPS"  IS 'Tiendas en una localidad de una ciudad';
COMMENT ON COLUMN "IGTP"."SHOPS"."ID" IS 'Identificador de la tienda';
